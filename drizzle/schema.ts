import { pgTable, pgSchema, index, foreignKey, integer, varchar, timestamp, text, serial, bigint, uuid, date } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const tigerDen = pgSchema("tiger_den");
export const contentTypeInTigerDen = tigerDen.enum("content_type", ['youtube_video', 'blog_post', 'case_study', 'website_content', 'third_party', 'other'])
export const sourceInTigerDen = tigerDen.enum("source", ['manual', 'csv_import', 'cms_api', 'asana_webhook'])


export const postInTigerDen = tigerDen.table("post", {
...
export const accountsInTigerDen = tigerDen.table("accounts", {
	userId: text().notNull(),
	type: text().notNull(),
...
export const sessionsInTigerDen = tigerDen.table("sessions", {
...
export const drizzleMigrationsInTigerDen = tigerDen.table("__drizzle_migrations", {
	id: serial().notNull(),
	hash: text().notNull(),
...
export const contentCampaignsInTigerDen = tigerDen.table("content_campaigns", {
...
export const contentItemsInTigerDen = tigerDen.table("content_items", {
	id: uuid().defaultRandom().notNull(),
	title: text().notNull(),
	...
	index("content_items_content_type_idx").using("btree", table.contentType.asc().nullsLast().op("enum_ops")),
	index("content_items_created_at_idx").using("btree", table.createdAt.asc().nullsLast().op("timestamp_ops")),
	index("content_items_current_url_idx").using("btree", table.currentUrl.asc().nullsLast().op("text_ops")),
	index("content_items_publish_date_idx").using("btree", table.publishDate.asc().nullsLast().op("date_ops")),
	foreignKey({
